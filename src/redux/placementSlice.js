  ...
  async (placementData, { rejectWithValue, dispatch }) => {
    try {
      const submissionValues = {
        ...placementData,
        startDate: placementData.startDate ? formatDateForAPI(placementData.startDate) : null,
        endDate: placementData.endDate ? formatDateForAPI(placementData.endDate) : null,
      };
      
      const response = await httpService.post('/candidate/placement/create-placement', submissionValues);
      ...
      ToastService.error(error.response?.data?.message || 'Failed to create placement. Please try again.');
      return rejectWithValue(error.response?.data?.message || 'Failed to create placement');
    }
  }
);
...
export const updatePlacement = createAsyncThunk(
  ...
  async ({ id, placementData }, { rejectWithValue, dispatch }) => {
    try {
      ...
      ToastService.error(error.response?.data?.message || 'Failed to update placement. Please try again.');
      return rejectWithValue(error.response?.data?.message || 'Failed to update placement');
    }
  }
);
...
export const deletePlacement = createAsyncThunk(
  ...
  async (id, { rejectWithValue, dispatch }) => {
    try {
      const response = await httpService.delete(`/candidate/placement/delete-placement/${id}`);
      ...
      ToastService.error(error.response?.data?.message || 'Failed to delete placement. Please try again.');
      return rejectWithValue(error.response?.data?.message || 'Failed to delete placement');
    }
  }
);
    ...
    setSelectedPlacement: (state, action) => {
      ...
      if (action.payload) {
        ...
        state.selectedPlacement = {
          ...placement,
          startDate: placement.startDate ? formatDateForAPI(placement.startDate) : '',
...
export const { resetPlacementState, setSelectedPlacement } = placementSlice.actions;

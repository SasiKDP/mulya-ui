...
const ClientForm = ({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (isEdit && initialData.supportingDocuments && Array.isArray(initialData.supportingDocuments)) {
  ...
  const validationSchema = Yup.object().shape({
    clientName: Yup.string().required("Client name is required").max(50, "Client name must be at most 35 characters"),
    clientAddress: Yup.string().nullable().max(250, "Client address must be at most 100 characters"),
  ...
  if (!Array.isArray(formInitialValues.clientSpocName) || formInitialValues.clientSpocName.length === 0) {
    formInitialValues.clientSpocName = [""];
  }
  if (!Array.isArray(formInitialValues.clientSpocEmailid) || formInitialValues.clientSpocEmailid.length === 0) {
    formInitialValues.clientSpocEmailid = [""];
  }
  if (!Array.isArray(formInitialValues.clientSpocMobileNumber) || formInitialValues.clientSpocMobileNumber.length === 0) {
    formInitialValues.clientSpocMobileNumber = [""];
  }
  if (!Array.isArray(formInitialValues.clientSpocLinkedin) || formInitialValues.clientSpocLinkedin.length === 0) {
  ...
  const handleFileChange = (event) => {
    ...
    const currentSize = files.reduce((sum, file) => sum + (file.isExisting ? 0 : file.size), 0);
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      files.filter(file => !file.isExisting).forEach((file) => {
          ...
          {({ values, errors, touched, resetForm, setFieldValue }) => (
            <Form>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <Typography variant="h6" color="primary" sx={{ mt: 1, mb: 1, fontWeight: 500, display: "flex", alignItems: "center" }}>
                    ...
                    <Box sx={{ mt: 2, display: "flex", flexWrap: "wrap", gap: 1 }}>
                      {files.length > 0 ? (
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ fontStyle: "italic" }}>
                          {isEdit ? "No documents available. Upload new documents." : "No files selected. Please upload at least one supporting document."}
                        </Typography>
                      )}
                    </Box>
                  </Paper>
                ...
                <Grid item xs={12}>
                  ...
                  <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="flex-end">
                    ...
                    <Button 
                      ...
                      sx={{ px: 4 }}
                    >
                      {isSubmitting ? "Submitting..." : isEdit ? "Update Client" : "Add Client"}
                    </Button>
                  </Stack>
                </Grid>
              </Grid>
